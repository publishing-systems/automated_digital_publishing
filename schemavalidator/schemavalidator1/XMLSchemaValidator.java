...
class SchemaEntityResolverLocal implements EntityResolver, LSResourceResolver
{
    public SchemaEntityResolverLocal(File entitiesResolverConfigFile, File schemataResolverConfigFile)
    {
        ...
        if (success == true)
        {
            success = this.entitiesResolverConfigFile.exists();
        }

        if (success == true)
        {
            success = this.entitiesResolverConfigFile.isFile();
        }

        if (success == true)
        {
            success = this.entitiesResolverConfigFile.canRead();
        }
            ...
            for (int i = 0; i < entityNodeListCount; i++)
            {
                ...
                for (int j = 0; j < entityChildNodeListCount; j++)
                {
                    ...
                    if (entityChildNode.getNodeName().equalsIgnoreCase("resolve") == true)
                    {
                        ...
                        {
                            System.out.println("schemavalidator1: '" + this.entitiesResolverConfigFile.getAbsolutePath() + "' contains a resolve entry with empty identifier.");
                            System.exit(-1);
                        }
                        ...
                        {
                            System.out.println("schemavalidator1: '" + referencedFile.getAbsolutePath() + "', referenced in '" + this.entitiesResolverConfigFile.getAbsolutePath() + "', doesn't exist.");
                            System.exit(-1);
                        }
                        
                        if (referencedFile.isFile() != true)
                        {
                            System.out.println("schemavalidator1: '" + referencedFile.getAbsolutePath() + "', referenced in '" + this.entitiesResolverConfigFile.getAbsolutePath() + "', isn't a file.");
                            System.exit(-1);
                        }
                        
                        if (referencedFile.canRead() != true)
                        {
                            System.out.println("schemavalidator1: '" + referencedFile.getAbsolutePath() + "', referenced in '" + this.entitiesResolverConfigFile.getAbsolutePath() + "', isn't readable.");
                            System.exit(-1);
                        }
                        ...
                        {
                            System.out.println("schemavalidator1: Identifier '" + identifier + "' configured twice in '" + this.entitiesResolverConfigFile.getAbsolutePath() + "'.");
                            System.exit(-1);
                        }
                    }
                }
            ...
            for (int i = 0; i < schemaNodeListCount; i++)
            {
                ...
                for (int j = 0; j < schemaChildNodeListCount; j++)
                {
                    ...
                    if (schemaChildNode.getNodeName().equalsIgnoreCase("resolve") == true)
                    {
                        ...
                        if (identifier.length() <= 0)
                        {
                            System.out.println("schemavalidator1: '" + this.schemataResolverConfigFile.getAbsolutePath() + "' contains a resolve entry with empty identifier.");
                            System.exit(-1);
                        }
                        ...
                        if (referencedFile.exists() != true)
                        {
                            System.out.println("schemavalidator1: '" + referencedFile.getAbsolutePath() + "', referenced in '" + this.schemataResolverConfigFile.getAbsolutePath() + "', doesn't exist.");
                            System.exit(-1);
                        }
                        
                        if (referencedFile.isFile() != true)
                        {
                            System.out.println("schemavalidator1: '" + referencedFile.getAbsolutePath() + "', referenced in '" + this.schemataResolverConfigFile.getAbsolutePath() + "', isn't a file.");
                            System.exit(-1);
                        }
                        
                        if (referencedFile.canRead() != true)
                        {
                            System.out.println("schemavalidator1: '" + referencedFile.getAbsolutePath() + "', referenced in '" + this.schemataResolverConfigFile.getAbsolutePath() + "', isn't readable.");
                            System.exit(-1);
                        }
                        ...
                        {
                            System.out.println("html2wordpress1: Identifier '" + identifier + "' configured twice in '" + this.schemataResolverConfigFile.getAbsolutePath() + "'.");
                            System.exit(-1);
                        }
                    }
                }
                                   ...
                                   String baseURI)
    {
        ...
        if (localSchema == null)
        {
            System.out.println("schemavalidator1: Can't resolve schema with type '" + type + "', namespace URI '" + namespaceURI + "', public ID '" + publicId + "', system ID '" + systemId + "', base URI '" + baseURI + "', no local copy configured in '" + this.schemataResolverConfigFile.getAbsolutePath() + "'.");
            System.exit(-1);
        }
    
        if (localSchema.exists() != true)
        {
            System.out.println("schemavalidator1: '" + localSchema.getAbsolutePath() + "', referenced in '" + this.schemataResolverConfigFile.getAbsolutePath() + "', doesn't exist.");
            System.exit(-1);
        }
        
        if (localSchema.isFile() != true)
        {
            System.out.println("schemavalidator1: '" + localSchema.getAbsolutePath() + "', referenced in '" + this.schemataResolverConfigFile.getAbsolutePath() + "', isn't a file.");
            System.exit(-1);
        }
        
        if (localSchema.canRead() != true)
        {
            System.out.println("schemavalidator1: '" + localSchema.getAbsolutePath() + "', referenced in '" + this.schemataResolverConfigFile.getAbsolutePath() + "', isn't readable.");
            System.exit(-1);
        }
    ...
    public InputSource resolveEntity(String publicId, String systemId)
    {
        ...
        if (localEntity == null)
        {
            System.out.println("schemavalidator1: Can't resolve entity with public ID '" + publicId + "', system ID '" + systemId + "', no local copy configured in '" + this.entitiesResolverConfigFile.getAbsolutePath() + "'.");
            System.exit(-1);
        }
    
        if (localEntity.exists() != true)
        {
            System.out.println("schemavalidator1: '" + localEntity.getAbsolutePath() + "', referenced in '" + this.entitiesResolverConfigFile.getAbsolutePath() + "', doesn't exist.");
            System.exit(-1);
        }
        
        if (localEntity.isFile() != true)
        {
            System.out.println("schemavalidator1: '" + localEntity.getAbsolutePath() + "', referenced in '" + this.entitiesResolverConfigFile.getAbsolutePath() + "', isn't a file.");
            System.exit(-1);
        }
        
        if (localEntity.canRead() != true)
        {
            System.out.println("schemavalidator1: '" + localEntity.getAbsolutePath() + "', referenced in '" + this.entitiesResolverConfigFile.getAbsolutePath() + "', isn't readable.");
            System.exit(-1);
        }
